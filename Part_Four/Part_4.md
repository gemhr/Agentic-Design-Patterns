...
## Chapter 20 Prioritization 优先级排序
...
在 LocalAgent 中，可以由 Router（路由器）或 Planner 给子任务设置：
...
Exploration and Discovery（探索与发现）指的是：

```text
当 Agent（智能体）不知道完整路径时，主动探索工具、知识、环境或候选方案，逐步发现完成任务所需的信息。
```
...
例如用户让 Agent 处理一个陌生 Excel 文件时，它可能先：
...
Environment Inspection（环境检查）：
先查看文件、目录、数据库或系统状态。

Candidate Exploration（候选方案探索）：
生成多个候选方案，再选择成本和风险更合适的方案。
```

### 和其他模式的关系

Exploration and Discovery 通常发生在 Planning（规划）之前或执行过程中：
...
它也和 Resource Aware Optimization（资源感知优化）有关，因为探索必须受预算限制；和 Prioritization（优先级排序）有关，因为要优先探索最可能解决核心问题的信息。

### LocalAgent 中的应用

在 LocalAgent（本地智能体项目）中，典型场景包括：

```text
处理陌生 Excel 前先检查 sheet 和列；
回答项目问题前先搜索知识库；
调用工具前先读取工具注册表；
工具失败后探索备用工具或替代参数；
代码审查前先检查项目目录和依赖结构。
...
```text
Exploration and Discovery 是 Agent 在环境或任务信息不完整时，主动检查工具、数据、知识和候选方案的能力。工程上探索应该以低风险、只读操作为主，并设置最大尝试次数、资源预算和停止条件，避免 Agent 无限搜索或盲目试错。
```

最核心一句：

```text
Exploration 负责发现可能性，Planning 负责选择和安排执行路径。
```
...
```text
Agent 不仅要能完成任务，还要能够被评估、受约束、会协作、懂资源取舍，并在复杂环境中做出合理决策。
```
...
```text
第一章：Agent 会执行任务；
第二章：Agent 能长期运行和持续改进；
第三章：Agent 出错可恢复、风险可确认、答案有依据；
第四章：Agent 可评估、可控制、可协作、可优化。
```

第四章包含七个模式，并按照面试重要性分成三个梯队。
...
Guardrails / Safety Patterns（护栏 / 安全模式）；
Inter-Agent Communication（智能体间通信）。
```

这三个模式最能体现 Agent 工程项目和普通 Demo（演示项目）的区别。

#### Evaluation and Monitoring（评估与监控）

Evaluation（评估）用于判断 Agent 改动后有没有变好，Monitoring（监控）用于观察 Agent 上线后是否稳定。

可以概括为：

```text
Evaluation 负责上线前判断“效果好不好”；
Monitoring 负责上线后判断“运行稳不稳”。
```

Agent 不能只依靠人工体验评估，因为人工测试存在：
...
只看最终答案，只能知道“错没错”；分模块评估才能知道“错在哪”。

需要分别评估：

```text
Router Accuracy（路由正确率）；
...
Approval Trigger Accuracy（审批触发准确率）。
```
...
Guardrails 用于限制 Agent 的行为边界，防止越权、泄密、误操作和资源失控。

它的核心不是让 Agent 不能做事，而是：

```text
让 Agent 只能在规则、权限和审批范围内安全地做事。
```

Agent 比普通聊天模型更需要护栏，因为它可能：
...
Planning Guardrails（规划护栏）；
Retrieval Guardrails（检索护栏）；
...
安全不能只靠 prompt（提示词），因为 prompt 是软约束，可能被绕过，也可能因为模型误解任务而失效。
...
```text
检索文档是 data（数据），不是 instruction（指令）。
```

文档不能改变系统规则、绕过权限或直接触发工具调用。

护栏违规也要进入评估闭环：

```text
Guardrail Violation（护栏违规）
→ trace
→ bad case
→ 修复规则或工具元数据
→ 回归测试。
...
```text
Agent 能做什么，由工具决定；
Agent 允许做什么，由 Guardrails 决定。
```

#### Inter-Agent Communication（智能体间通信）

Inter-Agent Communication 关注多个 Agent 如何交换：
...
它和 Multi-Agent Collaboration（多智能体协作）的区别是：

```text
Multi-Agent Collaboration 解决“哪些 Agent 一起工作”；
Inter-Agent Communication 解决“它们如何工作”。
```
...
```text
控制流走 Orchestrator（编排器）；
数据流走 Shared State；
观察流走 Event Stream。
```

Agent 之间不应该只传自由文本，而应设计统一对象。

AgentMessage（智能体消息）负责描述：
...
无法裁决时，应交给 Reviewer（审查器）或 Human-in-the-Loop。

为避免 Context Explosion（上下文爆炸），应遵循：
...
```text
多 Agent 的难点不是分几个角色，而是怎么通信、怎么同步、怎么聚合、怎么恢复。
```
...
这两个模式重点掌握工程取舍和基本落地方式，不需要做成单独复杂系统。
...
```text
如何根据任务复杂度和系统限制，动态控制模型、上下文、工具、检索、并发和执行深度。
```
...
复杂任务：
Planning + RAG + Tool Use。

高价值复杂任务：
多 Agent + Reflection + Verifier（验证器）。
...
```text
Model Routing（模型路由）决定用什么模型；
Profile（执行档位）决定走多深；
Budget（预算）决定最多消耗多少资源。
```
...
```text
Cache（缓存）：避免重复计算；
Batch Processing（批处理）：提高大量同类任务吞吐；
Parallelization（并行化）：缩短独立任务总耗时；
Early Stop（提前停止）：避免无收益执行。
```

资源优化不能只看成本或延迟，还要同时关注：
...
```text
在满足质量底线和安全底线的前提下，尽量降低成本与延迟。
```

最核心结论：

```text
资源优化不是使用最少资源，而是使用刚好足够的资源可靠完成任务。
```

#### Reasoning Techniques（推理技术）

Reasoning Techniques 关注：

```text
如何让 Agent 在复杂任务中完成问题分解、行动选择、证据使用、结果验证和错误修正。
```
...
复杂任务：
Planning + ReAct + 多方案比较 + Verifier + Reflection。
```

工程上更推荐结构化推理，而不是记录大量自由推理文本。

例如：

```text
Router 返回：
route、confidence、task_complexity。

Planner 返回：
steps、dependencies、stop_condition。

Agent 返回：
summary、evidence、warnings。

Verifier 返回：
passed、issues、revision_required。
```
...
Profile 分级；
程序级 Budget；
明确 Stop Condition（停止条件）；
Early Stop；
重复调用检测；
限制 Reflection 和 Replanning（重新规划）；
优先使用程序验证。
```
...
```text
有限资源下，先保证安全和核心目标，再处理普通优化与可选任务。
```

#### Exploration and Discovery（探索与发现）

Exploration and Discovery 解决：

```text
当 Agent 不知道完整路径时，如何主动探索工具、环境、知识和候选方案。
```

典型场景：

```text
陌生 Excel 文件先查看 sheet 和列；
代码审查前先检查目录结构；
回答问题前先检索知识库；
工具失败后查找备用工具或参数；
规划前先确认环境能力。
...
```text
Exploration 负责发现可能路径；
Planning 负责选择并安排执行路径。
```

### 第四章七个模式之间的关系

可以把第四章理解成一套完整的 Agent 工程控制体系：
...
```text
第四章主要解决 Agent 工程化中的评估、安全、协作和资源控制问题。

首先，我会通过 Evaluation and Monitoring 建立测试集、trace、指标、bad case 和回归体系，判断 Agent 改动是否有效以及线上是否稳定。

其次，通过 Guardrails 把安全边界固化到程序中，包括工具白名单、schema 校验、角色和路径权限、风险等级、人工审批、dry run、备份和结果校验，而不是只依赖 prompt。

在多 Agent 场景下，我会使用 Orchestrator 统一调度，通过 AgentMessage、AgentResult、Shared State 和事件流传递任务、状态、结果和错误，并处理并行、冲突和上下文裁剪。

资源方面，我会根据任务复杂度选择 fast、balanced 或 deep Profile，同时限制模型调用、工具调用、检索次数、并发、上下文和执行步数。

推理方面，简单任务直接回答，复杂任务才进行问题分解、工具增强、验证和反思，并设置停止条件避免过度推理。

Prioritization 和 Exploration 则用于在资源有限和信息不足时，先保证安全与核心目标，再探索必要信息并选择执行路径。
```
...
```text
Evaluation and Monitoring 让 Agent 的效果可量化、问题可定位、改动可回归。

Guardrails / Safety Patterns 让 Agent 的能力始终处于权限、规则和审批边界内。

Inter-Agent Communication 让多个 Agent 通过结构化消息、共享状态和事件机制稳定协作。
```

第二梯队可以概括为：

```text
Resource Aware Optimization 决定投入多少资源；
Reasoning Techniques 决定如何分析和完成任务。
```
...
Exploration and Discovery 决定信息不足时先探索什么。
```

最终可以浓缩成一句话：

```text
第四章的核心，是让 Agent 从“能够完成任务”，升级为“效果可评估、行为可控制、协作可追踪、资源可优化”的工程系统。
```
